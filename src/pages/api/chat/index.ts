...
async function fetchRecommendedCases(message: string, careEnvironment: string) {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  const { message, careEnvironment } = req.body;
  const careEnvironmentLabel = careEnvironment === "first_visit" ? "첫 진료" : "입원 중";
  try {
    // 1️⃣ 추천 API 호출 → 증례 추천
    const recommendedCases = await fetchRecommendedCases(message, careEnvironment);
...
- 반드시 아래 두 가지 정보를 포함한 아주 짧은 한 문장으로만 안내할 것:
  1. 현재 진료 환경 (반드시 명시, 예: 첫 진료, 입원 중 등)
  2. 추천된 증례 개수
- 문장 예시: "✅ 첫 진료 환경에서 총 N건의 유사 증례가 추천되었습니다."
- 반드시 한 문장으로 작성하고, ✅ 이모지 꼭 포함
- 절대 추가 설명, 증례 분석, 의학 정보, 조언 등을 포함하지 말 것.


사용자 질문: ${message}

진료 환경: ${careEnvironmentLabel}
